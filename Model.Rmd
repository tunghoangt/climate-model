...
# might need to include date structure to chart model performance over time
...
for( loc in locations ) {
  ...
  hour       <- c(hour, ncvar_get(open_file, 'hour'))
...
df    <- data.frame(bright_3.7 = bright_3.7, bright_8.6 = bright_8.6, bright_11 = bright_11,
                    ...
                    sst_act = sst_act, hour = hour)
# indicator of whether it's day time (used in #4 & #5)
day   <- c(7:17)
df$is_day   <- as.integer(df$hour %in% day)
```
...
### Part 4
Adding further variables and segmenting the model based on whether it's day or night we can enrich our model which then requires the estimation of 14 parameters.
...
So, ```alpha=1``` simplifies to the LASSO penalty and ```alpha=0``` = simplifies to the Ridge penalty. The ```glmnet()``` function requires our x variables denoted as a matrix, so let's perform some pre-processing to ensure it runs smoothly.

```{r model_prep}
# you should consider splitting into test & train sets here
# asks for independent valdiation, also this dataset is massive
...
y     <- as.matrix(df$sst_act, ncol = 1)
```

Now, we can fit both our LASSO and Ridge models. 

```{r fit}
# fit models
fit_lasso <- glmnet::glmnet(X, y, alpha = 1)
fit_ridge <- glmnet::glmnet(X, y, alpha = 0)
```

Let's take a look at the step-wise progression plots use the $L_1$ and $L_2$ norm penalties.

```{r pred}
# make predictions
sst_lasso <- predict(fit_lasso, X, type = "response")
sst_ridge <- predict(fit_ridge, X, type = "response")
```

What about their prediction accuracy? How is it compared to the error from the operational model?

```{r error}
# create a prediction/performance df to use in the succeeding visualizations

rmse <- function(y_act, y_pred) {
  return( length(y_pred)^-1 * sum((y_pred - y_act)^2) )
}

print(list(
  base_error = rmse(sst_act, sst_reg),
  lasso_error = rmse(sst_act, sst_lasso),
  ridge_error = rmse(sst_act, sst_ridge)
))
```

What do the pathways for both penalties look like?
