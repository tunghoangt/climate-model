...
Now, let's permutate the channels and see their performance.

```{r}
#Setting up pair of I and J for the model
x          <- c('bright_3.7', 'bright_4', 'bright_8.6', 'bright_11', 'bright_12')
p          <- gtools::permutations( n = 5, r = 2, v = x)
mse.models.is <- vector("list", length = nrow(p))
mse.models.os <- vector("list", length = nrow(p))
for (index in 1:nrow(p)) {
  formula             <- as.formula(paste("sst_act ~", toString(p[index,1]),
                                   "+I(",toString(p[index,1]),"-",toString(p[index,2]),"):sst_ref", sep=""))
  linear.model        <- lm(formula = formula, data = df[train_mask,])
  prediction          <- predict(linear.model, df[test_mask,], type = "response")
  mse.models.is[[index]] <- mean(residuals(linear.model)^2)
  mse.models.os[[index]] <- mean( (prediction - df[test_mask,]$sst_act)^2 )
}
rm(linear.model, X_test, prediction)  # memory management
# operational model error
op_error <- mean( (df[test_mask,]$sst_act - df[test_mask,]$sst_reg)^2 )
op_error
```

Now, let's see which model minimizes in-sample and out-of-sample performance:

```{r}
plot(1:20, mse.models.is, type = "b", main = "In-Sample Performance", xlab = "Index")
plot(1:20, mse.models.os, type = "b", main = "Out-of-Sample Performance", xlab = "Index")
```

### Part 3

To perform a subset selection of the predcitors for the model below, 
...
Due to our model consist of three predictors and two of the predictors is the difference between two variables. To calculate the total number of variables in our data, it is necessary to violate the Best subset selection algorithm. This algorithm is limited to analyze sets of pairs. It does not hold for pair repetition. In our model each variable is used twiced if and only if both the variables have different wavelengths. In other words, the model stablishes a different order each time a variable is used. To assert the total of possible models, mathematically this can be expressed as:
...
To account for the order of each variable used, it is comvenient to estimate the amount of possible by permutation instead of combinations. In total we will have 20 possible models, from which the model with the highest adjusted $R^2_{adj}$ will be selected. 
